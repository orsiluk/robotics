...
while notThere
    ...
    while(converged == 0 && n < maxNumOfIterations) %%particle filter loop
        ...
        if (distX>(num*0.85) && distY>(num*0.85))
...
%     for i =1:size(nodes)
%         c = 0;
%         conectionsL = ismember(nodes(:,i),edges(1,:,:));
%         conectionsR = ismember(nodes(:,i),edges(2,:,:));
%         connections = conectionsL+conectionsR;
%         for j = 1:size(connections)
%             if connections(j) == 1
%                 c =c+1;
%                 distToNode(j) = pdist([nodes(:,i); edges(2,:,j)],'euclidean');
%             elseif connectionsR(j) == 1
%                 c =c+1;
%                 distToNode(j) = pdist([nodes(:,i); edges(1,:,j)],'euclidean');
%             end
%         end
%
%     end
